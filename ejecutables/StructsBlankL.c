...
void main ( void )
{
	...
	//tenParkingSectors enSector= enSector1; 
	...
	while(1)
	{
		...
		if( enSector < enMaxSectors )
		{
			if( u8InputAction == 1 )
			{//
				ShowAvailableSlots (astMyParks[enSector]);
			}
			else if( u8InputAction == 2 )
			{
				ShowNotAvailableSlots(astMyParks[enSector]);
			}
			else if( u8InputAction == 3 )
			{
				ShowEarnedMoney(astMyParks[enSector]);
			}
			else if( u8InputAction == 4 )
			{
				if(astMyParks[enSector].u8AvailableSlots > 0)
				{
					astMyParks[enSector].u8AvailableSlots = u8AddCarToSector(astMyParks[enSector]);
				}else{
					printf("Full Sector\n");
				}
			}
			...
			{
				if(astMyParks[enSector].u8AvailableSlots == astMyParks[enSector].u8MaxSlots)
				{
					printf("Empty Sector\n");
				}else{
					astMyParks[enSector].u8AvailableSlots = u8RemoveCarFromSector(astMyParks[enSector]);
					u8ImportPaymentOfSlot(astMyParks[enSector]);
				}
			}
			else{
...
/*F1*/
void ShowAvailableSlots ( tstParkingInfo enCurrentSector )
{
	uint8 u8OutAvailable=0;
	
	printf("Places Available: %d\n", enCurrentSector.u8AvailableSlots);
	
}

/*F2*/
void ShowNotAvailableSlots ( tstParkingInfo enCurrentSector )
{
	printf("Places Not Available: %d\n", enCurrentSector.u8MaxSlots - enCurrentSector.u8AvailableSlots);
}

/*F3*/
void ShowEarnedMoney ( tstParkingInfo enCurrentSector )
{
	printf("Earned Money: %d", enCurrentSector.u32MoneyEarn);
}
/*F4*/
uint8 u8AddCarToSector( tstParkingInfo enCurrentSector )
{
	enCurrentSector.u8AvailableSlots -= 1;	
	return enCurrentSector.u8AvailableSlots ;
}
/*F5*/
uint8 u8RemoveCarFromSector( tstParkingInfo enCurrentSector )
{
	enCurrentSector.u8AvailableSlots += 1;
	return enCurrentSector.u8AvailableSlots;
}
/*F6*/
void u8ImportPaymentOfSlot( tstParkingInfo enCurrentSector )
{
	astMyParks[enSector].u32MoneyEarn += 5;
}